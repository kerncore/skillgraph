/**
 * Adaptive output budget for skillgraph_explore (#185).
 *
 ...
 */
describe('skillgraph_explore output respects the adaptive budget', () => {
  let testDir: string;
  let cg: SkillGraph;
  let handler: ToolHandler;

  beforeAll(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'skillgraph-explore-budget-'));
  ...
  it('keeps total output under the small-project cap', async () => {
    const result = await handler.execute('skillgraph_explore', { query: 'Session method helper' });
  ...
  it('prefixes source lines with line numbers by default (cat -n style)', async () => {
    delete process.env.SKILLGRAPH_EXPLORE_LINENUMS;
    const result = await handler.execute('skillgraph_explore', { query: 'Session method helper' });
    const text = result.content?.[0]?.text ?? '';
  ...
  it('omits line numbers when SKILLGRAPH_EXPLORE_LINENUMS=0', async () => {
    process.env.SKILLGRAPH_EXPLORE_LINENUMS = '0';
    try {
      const result = await handler.execute('skillgraph_explore', { query: 'Session method helper' });
      const text = result.content?.[0]?.text ?? '';
