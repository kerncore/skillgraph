...
import { SkillGraphConfig, DEFAULT_CONFIG } from '../src/types';
import { DatabaseConnection, getDatabasePath } from '../src/db';
import { QueryBuilder } from '../src/db/queries';

function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'skillgraph-security-test-'));
}
...
describe('MCP Input Validation', () => {
  ...
  it('should reject non-string query in skillgraph_search', async () => {
    const result = await handler.execute('skillgraph_search', { query: null });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('non-empty string');
  });

  it('should reject empty string query in skillgraph_search', async () => {
    const result = await handler.execute('skillgraph_search', { query: '' });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('non-empty string');
  });

  it('should accept valid query in skillgraph_search', async () => {
    const result = await handler.execute('skillgraph_search', { query: 'example' });
    expect(result.isError).toBeFalsy();
  });

  it('should clamp limit to valid range in skillgraph_search', async () => {
    // Extremely large limit should still work (clamped to 100)
    const result = await handler.execute('skillgraph_search', { query: 'example', limit: 999999 });
    expect(result.isError).toBeFalsy();
  });

  it('should reject non-string symbol in skillgraph_callers', async () => {
    const result = await handler.execute('skillgraph_callers', { symbol: 123 });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('non-empty string');
  });

  it('should reject non-string task in skillgraph_context', async () => {
    const result = await handler.execute('skillgraph_context', { task: undefined });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('non-empty string');
  });

  it('should reject non-string symbol in skillgraph_impact', async () => {
    const result = await handler.execute('skillgraph_impact', { symbol: [] });
    expect(result.isError).toBe(true);
  });

  it('should reject non-string symbol in skillgraph_node', async () => {
    const result = await handler.execute('skillgraph_node', { symbol: false });
    expect(result.isError).toBe(true);
  });

  it('should reject non-string symbol in skillgraph_callees', async () => {
    const result = await handler.execute('skillgraph_callees', { symbol: {} });
    expect(result.isError).toBe(true);
  });

  it('should handle NaN limit gracefully', async () => {
    const result = await handler.execute('skillgraph_search', { query: 'example', limit: 'abc' });
    expect(result.isError).toBeFalsy();
  });

  it('should handle negative limit gracefully', async () => {
    const result = await handler.execute('skillgraph_search', { query: 'example', limit: -5 });
    expect(result.isError).toBeFalsy();
  });
});
...
describe('Glob Matching (picomatch)', () => {
  const makeConfig = (include: string[], exclude: string[]): SkillGraphConfig => ({
    ...DEFAULT_CONFIG,
    rootDir: '/test',
    include,
    exclude,
