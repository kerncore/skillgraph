 ...
 * Foundation Tests
 *
 * Tests for the SkillGraph foundation layer.
 */
...
import { isInitialized, getSkillGraphDir, validateDirectory } from '../src/directory';
import { DatabaseConnection, getDatabasePath } from '../src/db';

// Create a temporary directory for each test
function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'skillgraph-test-'));
}
...
describe('SkillGraph Foundation', () => {
  let tempDir: string;
  ...
  describe('Initialization', () => {
    it('should initialize a new project', () => {
      const cg = SkillGraph.initSync(tempDir);

      expect(SkillGraph.isInitialized(tempDir)).toBe(true);
      expect(fs.existsSync(getSkillGraphDir(tempDir))).toBe(true);
      expect(fs.existsSync(getDatabasePath(tempDir))).toBe(true);

      cg.close();
    });

    it('should create .gitignore in .skillgraph directory', () => {
      const cg = SkillGraph.initSync(tempDir);

      const gitignorePath = path.join(getSkillGraphDir(tempDir), '.gitignore');
    ...
    it('should create config.json with defaults', () => {
      ...
      const configPath = path.join(getSkillGraphDir(tempDir), 'config.json');
    ...
    it('should throw if already initialized', () => {
      ...
      expect(() => SkillGraph.initSync(tempDir)).toThrow(/already initialized/i);
    });
  ...
  describe('Opening Projects', () => {
    it('should open an existing project', () => {
      // First initialize
      const cg1 = SkillGraph.initSync(tempDir);
      cg1.close();

      // Then open
      const cg2 = SkillGraph.openSync(tempDir);
      expect(cg2.getProjectRoot()).toBe(path.resolve(tempDir));
      cg2.close();
    });

    it('should throw if not initialized', () => {
      expect(() => SkillGraph.openSync(tempDir)).toThrow(/not initialized/i);
    });
  ...
  describe('Static Methods', () => {
    it('isInitialized should return false for new directory', () => {
      expect(SkillGraph.isInitialized(tempDir)).toBe(false);
    });
  ...
  describe('Uninitialize', () => {
    it('should remove .skillgraph directory', () => {
      const cg = SkillGraph.initSync(tempDir);

      cg.uninitialize();

      expect(fs.existsSync(getSkillGraphDir(tempDir))).toBe(false);
      expect(SkillGraph.isInitialized(tempDir)).toBe(false);
    });
  });

  describe('Close/Destroy', () => {
    it('should close database but keep .skillgraph directory', () => {
      const cg = SkillGraph.initSync(tempDir);
...
describe('Query Builder', () => {
  ...
  beforeEach(() => {
    tempDir = createTempDir();
    cg = SkillGraph.initSync(tempDir);
  });
