...
function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'skillgraph-pr19-test-'));
}
...
// =============================================================================
// Graph Traversal 'both' Direction Fix
// (requires better-sqlite3 - will use SkillGraph integration)
// =============================================================================
...
`);
    ...
    const cg = SkillGraph.initSync(testDir, {
...
describe('Database Layer Improvements', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should support batch insert of unresolved refs', async () => {
    ...
    const dbPath = path.join(testDir, 'skillgraph.db');
...
describe('CLI uninit', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should uninitialize a project via SkillGraph.uninitialize()', async () => {
    const SkillGraph = (await import('../src/index')).default;

    // Initialize
    const cg = SkillGraph.initSync(testDir);
    expect(SkillGraph.isInitialized(testDir)).toBe(true);

    // Uninitialize
    cg.uninitialize();

    // .skillgraph directory should be removed
    expect(SkillGraph.isInitialized(testDir)).toBe(false);
  });
});
