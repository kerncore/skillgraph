...
function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'skillgraph-installer-test-'));
}
...
describe('Installer Config Writer', () => {
  ...
  describe('readJsonFile error handling', () => {
    it('should return empty object for non-existent file', () => {
      ...
      expect(content.mcpServers.skillgraph).toBeDefined();
    });
    ...
    it('should handle corrupted JSON by creating backup', () => {
      ...
      // New file should be valid JSON with skillgraph config
      const content = JSON.parse(fs.readFileSync(claudeJson, 'utf-8'));
      expect(content.mcpServers.skillgraph).toBeDefined();
    ...
    it('should preserve existing valid config when adding skillgraph', () => {
      const claudeJson = path.join(tempDir, '.claude.json');
  ...
  describe('writeClaudeMd section replacement', () => {
    it('should create new CLAUDE.md with markers', () => {
      ...
      expect(content).toContain('<!-- SKILLGRAPH_START -->');
      expect(content).toContain('<!-- SKILLGRAPH_END -->');
      expect(content).toContain('## SkillGraph');
    });
    ...
    it('should replace marked section on update', () => {
      ...
      expect(final).toContain('## SkillGraph');
    });
    ...
    it('should not overwrite content after unmarked section with ### subsections', () => {
      // Create a CLAUDE.md with an unmarked SkillGraph section that has ### subsections
      ...
      const final = fs.readFileSync(claudeMdPath, 'utf-8');
      // The section after SkillGraph must be preserved
      ...
      // New SkillGraph content should be present with markers
      expect(final).toContain('<!-- SKILLGRAPH_START -->');
      expect(final).toContain('<!-- SKILLGRAPH_END -->');
    });
    ...
    it('should replace unmarked section without subsections', () => {
      ...
      fs.mkdirSync(claudeDir, { recursive: true });
      const claudeMdPath = path.join(claudeDir, 'CLAUDE.md');
      // Note: regex needs \n before ## SkillGraph, so prefix with another section
      fs.writeFileSync(claudeMdPath, [
        '## Intro',
        '',
        'Preamble',
