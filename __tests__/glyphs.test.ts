...
describe('supportsUnicode', () => {
  ...
  it('returns false on Windows by default (mojibake-prone consoles)', () => {
    withEnv({ SKILLGRAPH_ASCII: undefined, SKILLGRAPH_UNICODE: undefined, TERM: undefined }, () => {
      setPlatform('win32');
  ...
  it('returns false on Linux kernel console (TERM=linux)', () => {
    withEnv({ SKILLGRAPH_ASCII: undefined, SKILLGRAPH_UNICODE: undefined, TERM: 'linux' }, () => {
      setPlatform('linux');
      expect(supportsUnicode()).toBe(false);
    });
  });

  it('respects SKILLGRAPH_UNICODE=1 on Windows (opt-in escape hatch)', () => {
    withEnv({ SKILLGRAPH_UNICODE: '1', SKILLGRAPH_ASCII: undefined }, () => {
      setPlatform('win32');
      expect(supportsUnicode()).toBe(true);
    });
  });

  it('respects SKILLGRAPH_ASCII=1 on macOS (opt-out escape hatch)', () => {
    withEnv({ SKILLGRAPH_ASCII: '1', SKILLGRAPH_UNICODE: undefined }, () => {
      setPlatform('darwin');
      expect(supportsUnicode()).toBe(false);
    });
  });

  it('SKILLGRAPH_ASCII takes precedence over SKILLGRAPH_UNICODE', () => {
    withEnv({ SKILLGRAPH_ASCII: '1', SKILLGRAPH_UNICODE: '1' }, () => {
      setPlatform('darwin');
      expect(supportsUnicode()).toBe(false);
    });
  });
...
describe('getGlyphs', () => {
  ...
  it('returns ASCII glyphs on Windows', () => {
    withEnv({ SKILLGRAPH_ASCII: undefined, SKILLGRAPH_UNICODE: undefined }, () => {
      setPlatform('win32');
      const g = getGlyphs();
