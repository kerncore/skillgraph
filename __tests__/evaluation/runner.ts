...
if (!fs.existsSync(path.join(resolvedPath, '.skillgraph', 'skillgraph.db'))) {
  console.error(`No .skillgraph/skillgraph.db found at ${resolvedPath}`);
  process.exit(1);
}

let skillgraphSha = 'unknown';
try {
  skillgraphSha = execSync('git rev-parse --short HEAD', { encoding: 'utf-8' }).trim();
} catch {}

console.log(`\nSkillGraph Eval — ${path.basename(resolvedPath)}`);
console.log(`Codebase: ${resolvedPath}`);
console.log(`Commit:   ${skillgraphSha}`);
console.log(`Cases:    ${testCases.length}`);
console.log('');

async function run() {
  const cg = SkillGraph.openSync(resolvedPath);
