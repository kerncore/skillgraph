 ...
 * @example
 * ```typescript
 * import { FileError, ParseError, setLogger, silentLogger } from 'skillgraph';
 *
...
/**
 * Base error class for all SkillGraph errors.
 *
 * All SkillGraph-specific errors extend this class, allowing you to catch
 * all SkillGraph errors with a single catch block.
 *
 * @example
 * ```typescript
 * try {
 *   await cg.indexAll();
 * } catch (error) {
 *   if (error instanceof SkillGraphError) {
 *     console.log(`SkillGraph error [${error.code}]: ${error.message}`);
 *   }
 * }
 * ```
 */
 ...
 */
export class FileError extends SkillGraphError {
 ...
 */
export class ParseError extends SkillGraphError {
  readonly filePath: string;
 ...
 */
export class DatabaseError extends SkillGraphError {
 ...
 */
export class SearchError extends SkillGraphError {
 ...
 */
export class VectorError extends SkillGraphError {
  constructor(message: string, operation: string, cause?: Error) {
 ...
 */
export class ConfigError extends SkillGraphError {
...
/**
 * Simple logger for SkillGraph operations
 *
...
export const defaultLogger: Logger = {
  debug(message: string, context?: Record<string, unknown>): void {
    if (process.env.SKILLGRAPH_DEBUG) {
      console.debug(`[SkillGraph] ${message}`, context ?? '');
    }
  },
  warn(message: string, context?: Record<string, unknown>): void {
    console.warn(`[SkillGraph] ${message}`, context ?? '');
  },
  error(message: string, context?: Record<string, unknown>): void {
    console.error(`[SkillGraph] ${message}`, context ?? '');
  },
};
