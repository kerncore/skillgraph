 ...
 * reliably crashes SkillGraph with `Fatal process out of memory: Zone`
...
/**
 * Build the bordered banner shown when SkillGraph detects an
...
export function buildNode25BlockBanner(nodeVersion: string): string {
  ...
  return [
    sep,
    `[SkillGraph] Unsupported Node.js version: ${nodeVersion}`,
    sep,
    'Node.js 25.x has a V8 WASM JIT (turboshaft) Zone allocator bug that',
    'crashes with `Fatal process out of memory: Zone` when SkillGraph',
    'compiles tree-sitter grammars. SkillGraph WILL crash on this Node',
    'version mid-indexing. See https://github.com/colbymchenry/skillgraph/issues/81',
    '',
    ...
    '  SKILLGRAPH_ALLOW_UNSAFE_NODE=1 skillgraph ...',
    sep,
  ].join('\n');
}
