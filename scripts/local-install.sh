#!/usr/bin/env bash
# Build the current branch and link it as the global `skillgraph` for
...
if [ "${1:-}" = "--undo" ]; then
  ...
  npm install -g "${PKG}"
  echo "done: global skillgraph -> $(command -v skillgraph)"
  exit 0
fi
...
LINKED=$(command -v skillgraph || echo "(not on PATH)")
echo
echo "✓ global skillgraph now points to this branch"
echo "  binary:  ${LINKED}"
echo "  branch:  ${BRANCH}"
